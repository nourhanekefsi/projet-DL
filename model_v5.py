...
def generator_loss(disc_generated_output, gen_output, target, vgg_model):
    ...
    total_loss = gan_loss + (100 * l1) + (100 * c_loss) + (100 * s_loss)  # Adjust weights as needed
    return total_loss
...
# Compile the models
with strategy.scope():
    # Build the generator and discriminator inside the strategy scope
    generator = build_generator()
    discriminator = build_discriminator()
    generator_optimizer = Adam(4e-4, beta_1=0.5)
    discriminator_optimizer = Adam(1e-6, beta_1=0.5)
...
# Modify the training loop to work with `strategy`
def train(dataset, epochs):
    for epoch in range(epochs):
        for input_image, target in dataset:
            # Distribute the training step
            gen_loss, disc_loss, generated_image = strategy.run(train_step, args=(input_image, target))
        ...
        if epoch % 5 == 0:
            generator.save(f"/kaggle/working/model/generator_{epoch + 1}.h5")
            discriminator.save(f"/kaggle/working/model/discriminator_{epoch + 1}.h5")
...
dataset = dataset.map(data_augmentation).shuffle(BUFFER_SIZE).batch(BATCH_SIZE)
